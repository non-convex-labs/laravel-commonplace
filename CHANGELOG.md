...
- MCP tool errors that bubble up as `QueryException` now redact to `Database error: SQLSTATE[<code>]` instead of leaking DB host/port/database, the parameterized SQL trace, or PDO's `DETAIL:` row data. The redaction also covers `LostConnectionException` ("Database connection lost.") and bare `PDOException` including `DeadlockException` ("Database error."), neither of which the `QueryException` branch catches because they inherit from `LogicException` / `PDOException` rather than `QueryException`. Operators still see the full exception via `report()`. ([#115](https://github.com/non-convex-labs/laravel-commonplace/pull/119), [#118](https://github.com/non-convex-labs/laravel-commonplace/issues/118))
