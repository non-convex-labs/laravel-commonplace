...
- **Visibility scope.** Every read goes through `Note::accessibleBy($user)` (owned OR `visibility=public` OR present in `commonplace_shares`). See [model-relationships.md → Visibility model](../model-relationships.md#visibility-model-how-accessibleby-works). The enumeration defense — missing and inaccessible notes both surfacing as `Note not found.` — is uniform on the MCP read tools and the public-read route (`GET /{prefix}/public/{path}` 404s in either case, [mcp-tools.md → Visibility model](../mcp-tools.md#visibility-model)). The **authenticated web surface** is asymmetric by current implementation: `GET /{prefix}/{path}` returns 403 for an inaccessible existing note (`AuthorizationException` → `abort(403)`) and falls through to the folder browser (HTTP 200) for a non-existent path. A logged-in caller can therefore tell "exists but I can't read it" from "doesn't exist." The folder-browser fallback is a feature (paths can be either notes or folders); closing the asymmetry would require collapsing the 403 path into the folder browser too and is tracked separately — see the issue listed under "open divergences" below.
