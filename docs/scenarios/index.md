...
_None._ (#118 closed by the fail-closed allowlist sanitiser — see below.)
...
- [#118](https://github.com/non-convex-labs/laravel-commonplace/issues/118) → S-AI-25 (info disclosure, broader scope) — `publicMessageFor()` is now **fail-closed**. Unmarked Throwables collapse to a fixed `"The tool failed to complete the request."` instead of passing `getMessage()` through verbatim, so `ErrorException` no longer leaks filesystem paths, HTTP client exceptions no longer leak internal URLs or bearer-token query strings, `ModelNotFoundException` no longer leaks the consumer's model class name, and any Doctrine DBAL exception a consumer pulls in is covered by the same default. Tool authors opt their own exception classes into wire-visible messages by implementing the new `NonConvexLabs\Commonplace\Exceptions\PublicMessage` marker interface; the in-package `PgvectorDriverNotReady` is the first opt-in. Operators still see the full exception via `report()`, and a `Log::warning('mcp.envelope.redacted', ['class' => ...])` breadcrumb is emitted at the catch site so it's grep-able which class needs marking next.
