...
- **Visibility scope.** Every read goes through `Note::accessibleBy($user)` (owned OR `visibility=public` OR present in `commonplace_shares`). See [model-relationships.md → Visibility model](../model-relationships.md#visibility-model-how-accessibleby-works). The enumeration defense — missing and inaccessible notes both surfacing as `Note not found.` — is uniform on the MCP read tools and the public-read route (`GET /{prefix}/public/{path}` 404s in either case, [mcp-tools.md → Visibility model](../mcp-tools.md#visibility-model)). The **authenticated web surface** is asymmetric. The catch-all show route `GET /{prefix}/{path}` returns 403 for an inaccessible existing note and 200 (folder-browser fallback) for a non-existent path. The raw / download routes `GET /{prefix}/raw/{path}` and `GET /{prefix}/download/{path}` return 403 for inaccessible and 404 for non-existent — symmetric to each other, but neither matches the canonical `Note not found.` collapse. A logged-in caller can tell "exists but I can't read it" from "doesn't exist" on any of these. The folder fallback is intentional — paths can be notes or folders. Behavior fix to close the asymmetry is tracked in [#123](https://github.com/non-convex-labs/laravel-commonplace/issues/123).
...
- [#121](https://github.com/non-convex-labs/laravel-commonplace/issues/121) → theming iteration loop — `AssetController` emits `Cache-Control: public, max-age=3600` unconditionally; in dev a consumer's CSS edits get masked by browser cache for up to an hour. Followup from #116; emit `no-store` when `APP_DEBUG` / local env.
- [#122](https://github.com/non-convex-labs/laravel-commonplace/issues/122) → JS bundled-content positive marker — `AssetControllerTest::test_js_route_ignores_resource_path_and_serves_bundled_copy` only asserts the published marker is absent; an empty or whitespace-only response would also pass. Add a stable header marker to `commonplace.js` and assert on it positively. Followup from #116.
- [#123](https://github.com/non-convex-labs/laravel-commonplace/issues/123) → "Visibility scope" enumeration leak (behavior side) — collapse the authenticated `show` controller's `AuthorizationException` branch into the folder-browser fallthrough so 403 and 200-folder-fallback become byte-identical. Followup from the #117 doc carve-out below.
