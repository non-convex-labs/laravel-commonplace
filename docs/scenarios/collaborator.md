...
**Expected.** Step 2 throws `Illuminate\Auth\Access\AuthorizationException` (\"You do not have access to this note.\"). The note still exists, so the service raises the access-denied class, not `ModelNotFoundException`. The service layer is informative because in-process callers are trusted. All HTTP read surfaces collapse "inaccessible" and "missing" into the same response shape — the MCP read tools and the public-read route return `Note not found.`, the authenticated catch-all show route falls through to the folder browser, and the raw / download / history / graph / suggested-links routes 404. See the "Visibility scope" invariant in [scenarios/index.md](index.md#cross-cutting-invariants) and [S-NOTE-03](note-taker.md#s-note-03--read-a-note-requires-visibility-access).
